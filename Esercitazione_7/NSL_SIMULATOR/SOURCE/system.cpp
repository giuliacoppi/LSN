...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    if (_particle(i).getspin() == 1){
      double P_Gibbs = 1. / (1. + exp(-2. * _beta* _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() )));
      double random_number = _rnd.Rannyu();
      if(random_number > P_Gibbs){
        _particle(i).flip();
      }
    }
    else{   //spin=-1
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4, proportional to sqrt(T)
        _measure_pofv = true;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if (bin < _n_bins){
          double Delta_V = 4.*M_PI /3. * (pow((bin+1)*_bin_size, 3) - pow(bin*_bin_size, 3));
          _measurement(_index_gofr + bin) += 2.0/(_rho*(double)_npart*Delta_V);
        }    
        if(dr < _r_cut){
  ...
  if(_measure_pofv){
    double v;
    for (int i=0; i<_npart; i++){
      v = sqrt(dot(_particle(i).getvelocity(), _particle(i).getvelocity())); //compute |v|^2
      bin = int(v/_bin_size_v);
      if (bin < _n_bins_v){
        _measurement(_index_pofv + bin) += 1.0; //Put in the right bin
      }
    }
    for (int i = 0; i < _n_bins_v; i++){
      _measurement(_index_pofv + i) /= (double)_npart * _bin_size_v; //Normalization
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure and _sim_type==1) _measurement[_index_pressure] = _rho * _temp + (_ptail*_npart + 48.0*virial/3.0)/_volume;
  if (_measure_pressure and _sim_type==0) _measurement[_index_pressure] = _rho * (2./3.) * kenergy_temp + (_ptail*_npart + 48.0*virial/3.0)/_volume;
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    _measurement(_index_chi) = _beta * (magnetization * magnetization) / double(_npart);  
  }
...
void System :: averages(int blk){
  ...
  if (_measure_gofr){
    ...
    for (int i = 0; i < _n_bins; i++){
      ...
      coutf << setw(12) << (i + 0.5) * _bin_size << setw(12) << sum_average/double(blk) << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
    coutf.close();
  }
  // POFV //////////////////////////////////////////////////////////////////////
  if (_measure_pofv){
    ...
    for (int i = 0; i < _n_bins_v; i++){
      ...
      coutf << setw(12) << (i + 0.5) * _bin_size_v << setw(12) << sum_average/double(blk) << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
    coutf.close();
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  ...
  if (_measure_cv && _measure_tenergy){
    ...
    average = (_beta * _beta) * (_average(_index_cv) - (_average(_index_tenergy) * _average(_index_tenergy)))*_npart; //variance
